...
namespace SWS
{
	...
	public class WaypointManager : MonoBehaviour
	{
		...
		{
			string[] array = Paths.Where(delegate (KeyValuePair<string, PathManager> p)
			{
				KeyValuePair<string, PathManager> keyValuePair = p;
				return keyValuePair.Value == null;
			}).Select(delegate (KeyValuePair<string, PathManager> p)
			{
				KeyValuePair<string, PathManager> keyValuePair = p;
		...
		public static List<Vector3> SmoothCurve(List<Vector3> pathToCurve, int interpolations)
		{
			...
			int num = count * Mathf.RoundToInt(interpolations) - 1;
			List<Vector3> list = new List<Vector3>(num);
			for (int i = 0; i < num + 1; i++)
			{
				float num2 = Mathf.InverseLerp(0f, num, i);
